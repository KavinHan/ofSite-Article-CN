...
这里的代码具有大规模跨平台兼容的可能性。至今我们支持的操作系统有五个 (Windows, OSX, Linux, iOS, Android) 和四个 IDEs (XCode, Code::Blocks, Visual Studio 和 Eclipse). The API is designed to be minimal and easy to grasp.

简单来说, openFrameworks is a tool that makes it much easier to make things with code. We find it super useful, and we hope you do too.

openFrameworks 会长期性由 http://thesystemis.com/[Zach Lieberman], http://muonics.net/[Theodore Watson], 和 http://arturocastro.net/[Arturo Castro]开发, with help from the http://www.openframeworks.cc/community/[OF community]. openFrameworks is indebted to two significant precursors: the http://processing.org/[Processing development environment], created by http://reas.com/[Casey Reas], http://benfry.com/[Ben Fry] and the Processing community; and the ACU Toolkit, a privately distributed C++ library developed by Ben Fry and others in the http://acg.media.mit.edu/[MIT Media Lab's Aesthetics and Computation Group].
__________________________
